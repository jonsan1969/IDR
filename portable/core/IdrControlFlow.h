...
inline bool AnalyzeBoundedControlFlow(DWord entryPoint,
    ...
    const auto traceLinear = [&](DWord startAddress, std::vector<TraceInstruction> &trace) {
        ...
        for (std::size_t i = 0; i < options.traceLimit; ++i) {
            ...
            trace.push_back({address, decoded.length, decoded.mnemonic, decoded.disasm});
            ...
            if ((decoded.call || decoded.branch) && decoded.target != 0) {
                ...
                if (targetOffset >= 0) {
                    ...
                    if (decoded.call) enqueueCandidate(decoded.target);
                }
            }

            if (decoded.ret || (decoded.branch && !decoded.conditional)) break;
            if (static_cast<unsigned int>(decoded.length) >
                (std::numeric_limits<DWord>::max)() - address)
                break;
            address += static_cast<DWord>(decoded.length);
        }
        return !trace.empty();
    };

    if (!traceLinear(entryPoint, result.entryTrace))
    ...
    while (!candidateQueue.empty() && result.candidates.size() < options.candidateLimit) {
        ...
        if (!traceLinear(candidate.address, candidate.instructions))
