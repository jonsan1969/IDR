...
$name = $src.IndexOf('TDecompiler::SimulateFloatInstruction(')
$start = if ($name -ge 0) { $src.LastIndexOf('void __fastcall ', $name) } else { -1 }
$end = if ($name -ge 0) { $src.IndexOf("`n//---------------------------------------------------------------------------", $name) } else { -1 }
if ($start -lt 0 -or $end -lt 0 -or $end -le $start) {
    throw "Decompiler float slice markers not found (name=$name start=$start end=$end)"
}
...
Set-Content -LiteralPath "$PSScriptRoot\generated\Decompiler.float.slice.cpp" -Value ($prefix + $body) -Encoding utf8
