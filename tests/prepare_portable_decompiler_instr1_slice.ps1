...
$name = $src.IndexOf('TDecompiler::SimulateInstr1(')
$start = if ($name -ge 0) { $src.LastIndexOf('void __fastcall ', $name) } else { -1 }
$end = if ($name -ge 0) { $src.IndexOf("`n//---------------------------------------------------------------------------", $name) } else { -1 }
if ($start -lt 0 -or $end -lt 0 -or $end -le $start) {
    throw "Decompiler instr1 slice markers not found (name=$name start=$start end=$end)"
}
...
$numericStringArgs = @(
    '_offset', '_item.IntValue', '_item1.IntValue', '_item2.IntValue', 'DisInfo.Immediate', 'DisInfo.Offset'
)
...
Set-Content -LiteralPath "$PSScriptRoot\generated\Decompiler.instr1.slice.cpp" -Value ($prefix + $body) -Encoding utf8
