...
$name = $src.IndexOf('TDecompiler::SimulateInstr2MemImm(')
$start = if ($name -ge 0) { $src.LastIndexOf('void __fastcall ', $name) } else { -1 }
$end = if ($name -ge 0) { $src.IndexOf("`n//---------------------------------------------------------------------------", $name) } else { -1 }
if ($start -lt 0 -or $end -lt 0 -or $end -le $start) {
    throw "Decompiler instr2-memimm slice markers not found (name=$name start=$start end=$end)"
}
...
Set-Content -LiteralPath "$PSScriptRoot\generated\Decompiler.instr2.memimm.slice.cpp" -Value ($prefix + $body) -Encoding utf8
