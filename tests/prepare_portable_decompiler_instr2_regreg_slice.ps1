...
$name = $src.IndexOf('TDecompiler::SimulateInstr2RegReg(')
$start = if ($name -ge 0) { $src.LastIndexOf('void __fastcall ', $name) } else { -1 }
$end = if ($name -ge 0) { $src.IndexOf("`n//---------------------------------------------------------------------------", $name) } else { -1 }
if ($start -lt 0 -or $end -lt 0 -or $end -le $start) {
    throw "Decompiler instr2-regreg slice markers not found (name=$name start=$start end=$end)"
}
...
# Reuse only String compatibility mappings already established by earlier
# instruction slices. Keep additions compiler-driven and expression-specific.
$numericStringArgs = @(
    '_offset', '_foffset', '_pow2', '_mod', '_size', '_idx', '_classSize', '_dotpos',
...
Set-Content -LiteralPath "$PSScriptRoot\generated\Decompiler.instr2.regreg.slice.cpp" -Value ($prefix + $body) -Encoding utf8
